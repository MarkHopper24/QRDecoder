...
# Function to close running instance at end of script
def closeSelf(processName):
    # Loop through all running processes
    for proc in psutil.process_iter():
        try:
            # If the process exists, was created before programStart, and has a different PID
...
while os.path.exists(capturedQR) == False:
    #Self destructing if we've been in the loop for > 5 minutes (E.G user launches app and leaves snipping tool without capturing anything.)
    if((time.time() - programStart) > 300):
        try:
            closeSelf('QRDecoder.exe')
        except:
            pass
...
field.configure(state="disabled", selectbackground="Yellow", selectforeground="Black")
